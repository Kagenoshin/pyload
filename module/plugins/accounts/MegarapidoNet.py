...
class MegarapidoNet(Account):
    ...
    LOGIN_LINK_PATTERN = re.compile(r'\.post\([\'"]([^\'"]*)[\'"], \{login: login, senha: senha\}', re.I)
    ...
    def login(self, user, data, req):
        ...
        response = req.load("http://megarapido.net/login")
        login_link = self.LOGIN_LINK_PATTERN.search(response)
        if login_link:
            login_link = login_link.group(1)
        else:
            login_link = "http://megarapido.net/painel_user/ajax/logar1.php"
        response = req.load(login_link, post=post_data, decode=True)
